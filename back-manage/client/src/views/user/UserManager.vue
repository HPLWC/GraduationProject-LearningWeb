    ...
    <div class="tables">
      <el-table
        ...
        <el-table-column
            ...
            width="100">
          <template slot-scope="scope">
            <span style="margin-left: 10px">{{ scope.row.is_used ? '正常使用' : '禁用中' }}</span>
          </template>
        </el-table-column>
        <el-table-column
            label="创建时间"
...
<!--                @click="handleEdit(scope.$index, scope.row)">编辑</el-button>-->
            <el-button
                size="mini"
                type="danger"
                @click="handleDelete(scope.$index, scope.row)">{{ scope.row.is_used ? '停用' : '启用' }}</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
