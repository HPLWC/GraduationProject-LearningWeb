...
<!--          <el-button type="primary" size ="small" icon="el-icon-edit-outline" @click='onAddMoney()'>添加</el-button>-->
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <router-link :to="`/course/section?infoId=${scope.row.id}`" style="margin-left: 10px">{{ scope.row.title }}</router-link>
          </template>
        </el-table-column>
        <el-table-column
            label="描述"
        ...
        <el-table-column
            ...
            width="150">
          <template slot-scope="scope">
            <span style="margin-left: 10px">{{ scope.row.courseType.title }}</span>
          </template>
        </el-table-column>
        <el-table-column
            label="播放次数"
            ...
            width="100">
          <template slot-scope="scope">
            <span style="margin-left: 10px">{{ scope.row.play_num }}</span>
          </template>
        </el-table-column>
        <el-table-column
            label="创建时间"
    ...
    <CourseInfoModal :dialong="dialong" :form="form" @UserData="userInfo"></CourseInfoModal>
  </div>
</template>
...
export default {
  ...
  methods: {
    userInfo() {
      let url = `/api/course/info/all?pageNum=${this.paginations.pageIndex}&pageSize=${this.paginations.pageSize}`
