...
<template>
  <div>
    <layout-header>
      <img v-if="userInfo.photo" :src="userInfo.photo" class="avatar m-r-20">
      <hpc-icon v-else name="defaultuser" :size="100" class="m-t-2" style="background: transparent;"></hpc-icon>
      <p>{{ userInfo.name }}</p>
    </layout-header>
...
class UserCourse extends Vue {
  ...
  /* vue-method */
  async getTheUserInfo (params) {
    const { data } = await this.$store.dispatch('getTheUserInfo', {
  ...
  async getAllInfo (params) {
    const { data } = await this.$store.dispatch('getTheUserCourseInfo', {
      ...params,
